...
public class RepasoHotel {
    ...
    public static Map<String, Double> calcularFacturas(Map<String, Map<Integer, Integer>> registros, Map<Integer, Double> precios) {
        ...
        for (String dni : registros.keySet()) {
            ...
            double total = 0;
            // Proceso todas las habitaciones
            Map<Integer, Integer> habitacionesOcupadasPorElDNI = registros.get(dni);
            for (Integer habitacion : habitacionesOcupadasPorElDNI.keySet()) {
                Double precio = precios.getOrDefault(habitacion, 0d);
                Integer noches = habitacionesOcupadasPorElDNI.get(habitacion);
                double importe = precio * noches;
                total += importe;
            }
            // Añado al listado de DNIs
            resultado.put(dni, total);
    ...
    public static Map<Integer, Integer> calcularOcupacionPorHabitacion(Map<String, Map<Integer, Integer>> registros) {
        ...
        for (String dni : registros.keySet()) {
            ...
            for (Integer habitacion : habitacionesOcupadasPorElDNI.keySet()) {
                ...
                // forma 1
                if (ranking.containsKey(habitacion)) {
                    ranking.put(habitacion, ranking.get(habitacion) + noches);
                } else {
                    ranking.put(habitacion, noches);
                }

                // forma 2 (más corta, más compleja)
