...
public class AnalizadorCiberseguridad {
    ...
    public static void main(String[] args) {
        ...
        nivelRiesgoZona.put("Despacho jefe", "ALTO");
        nivelRiesgoZona.put("Oficinas", "MEDIO");
        nivelRiesgoZona.put("Almacen", "MEDIO");
        ...
        // Sacamos los niveles de riesgo reales existentes (y únicos)
        Set<String> nivelesRiesgo = new HashSet<>(nivelRiesgoZona.values());
        System.out.println("Niveles de riesgo :: " + nivelesRiesgo);
        for (String nivel : nivelesRiesgo) {
            reporteRiesgo.put(nivel, new ArrayList<>());
        }
        for (Log log : logsUnicos) {
            // Primero sacamos el nivel de riesgo del log actual
            String ubicacion = log.ubicacion;
            String nivel = nivelRiesgoZona.getOrDefault(log.ubicacion, "BAJO");
            // Lo añadimos al listado correspondiente
            reporteRiesgo.get(nivel).add(log);
        }
        System.out.println("Reporte riesgo :: " + reporteRiesgo);
        ...
        for (Log log : logsUnicos) {
            if (log instanceof LogCamara) {
                idsRevisionVisual.add(log.idLog);
            }
        }
